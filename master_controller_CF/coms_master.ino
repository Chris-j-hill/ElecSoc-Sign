...
void init_i2c()      //initialise the pins for comms
{
  ...
  for (int ref = 0; ref < NO_ADDRESSES; ref++)
  {
    ...
    //pinMode(arduino1_enable[ref], OUTPUT);
  }
}
...
{ 
  ...
  make_frame(mode);
  Wire.beginTransmission(address); // transmit to device with address 1,2,3 and 4
  Wire.write(frame, frame[FRAME_LENGTH_BYTE]);              // send frame
...
void std_text_frame(){
  frame[FRAME_LENGTH_BYTE] = input_str.length() + HEADER_LENGTH +TRAILER_LENGTH; //frame length  (+5 for extra 5 bytes we add just here)
    ...
    frame[DATA_IDENTIFIER_BYTE] = mode + 48;                         //character conversion for mode value
   int k=0;
...
void update_brightness(){
  ...
  for (int alpha = 1; alpha<=NO_ADDRESSES; alpha++){    //send on to all arduinos
    ...
    Wire.beginTransmission(alpha); // transmit to device with address 1,2,3 and 4
    Wire.write(frame, frame[FRAME_LENGTH_BYTE]);              // send frame
    Wire.endTransmission();
    Wire.flush();
